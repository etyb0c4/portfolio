...
function sphere() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    const th = 2 * Math.PI * u, ph = Math.acos(2 * v - 1), r = 15 + (Math.random() - 0.5) * 1.5
...
function grid() {
  ...
  for (let i = 0; i < N; i++) {
    ...
    a[i*3] = (gx/cols - 0.5) * 52
    a[i*3+1] = (gy/rows - 0.5) * 30
    a[i*3+2] = Math.sin(gx*0.5)*Math.cos(gy*0.5) * 3
  }
  return a
}
function wave() {
  const a = new Float32Array(N * 3)
  for (let i = 0; i < N; i++) {
    const x = (Math.random()-0.5)*52, z = (Math.random()-0.5)*30
    a[i*3] = x; a[i*3+1] = Math.sin(x*0.25)*3 + Math.cos(z*0.3)*2.4; a[i*3+2] = z
  }
  return a
}
...
const FRAG = /* glsl */`
  ...
  void main(){
    ...
    gl_FragColor = vec4(col, a * (0.13 + 0.32*uHeat));
  }
`
...
export default function Scene() {
  ...
  useEffect(() => {
    ...
    const bloom = new UnrealBloomPass(new THREE.Vector2(innerWidth, innerHeight), 0.5, 0.35, 0.0)
    ...
    const tick = () => {
      ...
      uniforms.uHeat.value += ((Math.min(1, p * 0.8 + extraHeat) + (entered?0.15:0)) - uniforms.uHeat.value) * 0.05
      ...
      if (forms.length && points) {
        ...
        const k = (typeof location!=='undefined' && location.search.includes('snap')) ? 1 : (entered ? 0.09 : 0.02)
        ...
        for (let i = 0; i < N; i++) {
          ...
          const tz = A[ix+2] + (B[ix+2] - A[ix+2]) * f
          // gentle breathing
    ...
    return () => { cancelAnimationFrame(raf); removeEventListener('resize', resize); removeEventListener('pointermove', onMove); renderer.dispose(); geo.dispose(); mat.dispose(); composer.dispose() }
  }, [])
